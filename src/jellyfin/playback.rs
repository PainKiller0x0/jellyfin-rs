...
async fn media_streams_for_item(db: &AnyPool, item_id: &str) -> anyhow::Result<Vec<Value>> {
    let rows = sqlx::query("SELECT stream_index, stream_type, codec, language, title, bit_rate, width, height, channels, sample_rate, path, is_external FROM media_streams WHERE item_id = ? ORDER BY stream_index ASC").bind(item_id).fetch_all(db).await.with_context(|| format!("failed to list media streams for item: {item_id}"))?;
    rows.into_iter()
        .map(MediaStreamRow::from_row)
...
pub async fn subtitle_stream_path(
    ...
    let row = sqlx::query("SELECT path FROM media_streams WHERE item_id = ? AND stream_index = ? AND stream_type = 'Subtitle' AND is_external = 1").bind(item_id).bind(stream_index).fetch_optional(db).await.with_context(|| format!("failed to find subtitle stream: {item_id}:{stream_index}"))?;
    row.map(|row| row.try_get("path"))
        .transpose()
...
pub async fn hide_from_resume(
    ...
    let now = now_unix();
    match sqlx::query("INSERT INTO user_data (user_id, item_id, playback_position_ticks, updated_at) VALUES (?, ?, 0, ?) ON CONFLICT(user_id, item_id) DO UPDATE SET playback_position_ticks = 0, updated_at = ?")
        .bind(&user_id)
        .bind(&item_id)
        .bind(now)
        .bind(now)
...
pub async fn set_rating(
    ...
    let rating = body
        ...
        .or_else(|| body.get("Rating").and_then(Value::as_f64))
        .unwrap_or(0.0);
    let now = now_unix();
    match sqlx::query(
...
async fn playback_progress_inner(
    ...
    let Some(item_id) = body.get("ItemId").and_then(Value::as_str) else {
    ...
    let position_ticks = body
        ...
        .unwrap_or_default();
    let user_id = if let Some(user_id) = body.get("UserId").and_then(Value::as_str) {
        user_id.to_string()
    } else {
    ...
    let session = PlaybackSession {
        ...
        play_state: PlaybackState {
            ...
            is_paused: body
                ...
                .and_then(Value::as_bool)
                .unwrap_or(false),
            can_seek: body.get("CanSeek").and_then(Value::as_bool).unwrap_or(true),
        },
...
async fn upsert_playback_position(
    ...
    let now = now_unix();
    sqlx::query(r#"INSERT INTO user_data (user_id, item_id, playback_position_ticks, updated_at, last_played_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(user_id, item_id) DO UPDATE SET playback_position_ticks = excluded.playback_position_ticks, updated_at = excluded.updated_at, last_played_at = excluded.last_played_at"#)
        .bind(user_id).bind(item_id).bind(position_ticks.max(0)).bind(now).bind(now).execute(db).await.with_context(|| format!("failed to update playback position for item: {item_id}"))?;
    Ok(())
}
